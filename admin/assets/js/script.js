...
if($('.datatable').length > 0) {
    $('.datatable').DataTable({
        ...
        "language": {
            ...
            sLengthMenu: 'Row Per Page _MENU_ Entries',
            info: "_START_ - _END_ of _TOTAL_ items",
            emptyTable: "No users found matching your criteria", // Add this line
            paginate: {
                next: '<i class="isax isax-arrow-right-1"></i>',
                previous: '<i class="isax isax-arrow-left"></i> '
            },
        ...
        "initComplete": function(settings, json) {
            ...
            if (this.api().data().length === 0) {
                $(this.api().table().body()).html(
                    '<tr class="odd">' +
                    '<td valign="top" colspan="8" class="dataTables_empty">No users found matching your criteria</td>' +
                    '</tr>'
                );
            }
        }
...
if ($('#reportrange').length > 0) {
    ...
    const startFromUrl = urlParams.get("start_date") ? moment(urlParams.get("start_date")) : moment().subtract(29, "days");
    const endFromUrl = urlParams.get("end_date") ? moment(urlParams.get("end_date")) : moment();
    ...
    function report_range(start, end) {
        ...
        window.location.href = url.toString(); // Refresh with params
    }
    ...
    // Set text initially
    $("#reportrange span").html(startFromUrl.format("D MMM YY") + " - " + endFromUrl.format("D MMM YY"));
}
...
//     tableBody.append(addTable);

//     // Get current selected item_type (1 = product, 0 = service)
//     var itemType = $('input[name="item_type"]:checked').val();

//     // Load the dropdown for the newly added row
//     $.ajax({
//         url: 'process/get_productcategories_by_type.php',
//         type: 'POST',
//         data: { item_type: itemType },
//         success: function (data) {
//             // Populate ONLY the last added row's dropdown
//             $('.item-select').last().html(data);
//         }
//     });
// });
$(".add-invoice-data").on('click', function (e) {
    ...
    <tr>
        ...
        <td>
            <input type="text" class="form-control tax-rate" readonly>
...
<input type="hidden" class="tax-name" name="tax_name[]">
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            allOptions = allOptions.filter(function () {
                let val = $(this).val();
                return val === "" || !selectedProducts.includes(val);
            });

            // Append filtered options to the new row's dropdown
            tableBody.find('tr:last .item-select').html(allOptions);
        }
    });
});
